...
def write_report(results):
    ...
    if not slipped:
        ...
        for r in slipped:
            ...
            lines.append("- **Why it slipped:** the heuristic filter matches literal "
                         "keywords; this payload conveys the same intent without them.")
            lines.append("")
    lines.append("## Recommended fixes")
    lines.append("")
    lines.append("1. **Normalise before matching.** Lowercase, strip non-alphanumeric "
                 "noise, collapse spaced-out letters, and decode common encodings "
                 "(e.g. base64) before running the patterns. This closes EV-01, EV-02, EV-04.")
    lines.append("2. **Add a second layer: an LLM-based injection classifier.** A model "
                 "judges intent rather than spelling, catching paraphrases like EV-03 "
                 "that no keyword list will ever cover.")
    lines.append("3. **Lean on output validation.** EV-05 injects fake data, not "
                 "instructions — the input filter can't see the harm, but the output "
                 "guard's provenance check rejects any verdict citing a value that "
                 "isn't in the real readings.")
    lines.append("4. **Defence in depth.** No single layer is sufficient; the combination "
                 "of input screening + output validation is what makes the system "
                 "resilient.")
    lines.append("")
